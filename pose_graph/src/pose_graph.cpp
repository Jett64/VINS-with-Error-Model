...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    //     std::time_t now_time = std::chrono::system_clock::to_time_t(start_time);
    //     std::tm local_time = *std::localtime(&now_time);
    //     ossTime << local_time.tm_year + 1900 << "-"
    //             << local_time.tm_mon + 1 << "-"
    ...
    //     start_mark = false;
    // }
    // std::string result_path = VINS_RESULT_PATH;
    // result_path.erase(result_path.end() - 4, result_path.end());

    if (SAVE_LOOP_PATH)
    {
        // std::string file_name = result_path + "_" + ossTime.str() + ".csv";
        // ofstream loop_path_file(file_name, ios::app);
        ...
        // write result to tum file
        // file_name = result_path + "_tum_" + ossTime.str() + ".tum";
        ofstream loop_path_file_tum(VINS_RESULT_PATH, ios::app);
        loop_path_file_tum.setf(ios::fixed, ios::floatfield);
        loop_path_file_tum.precision(9);
        loop_path_file_tum << cur_kf->time_stamp << " ";
        loop_path_file_tum.precision(5);
        loop_path_file_tum  << P.x() << " "
              << P.y() << " "
              << P.z() << " "
              << Q.x() << " "
              << Q.y() << " "
...
//        printf("path p: %f, %f, %f\n",  P.x(),  P.z(),  P.y() );
        ...
        if (SAVE_LOOP_PATH)
        {
            ...
            loop_path_file_tum.setf(ios::fixed, ios::floatfield);
            loop_path_file_tum.precision(9);
            loop_path_file_tum << (*it)->time_stamp << " ";
            loop_path_file_tum.precision(5);
            loop_path_file_tum  << P.x() << " "
                  << P.y() << " "
                  << P.z() << " "
