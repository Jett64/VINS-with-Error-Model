...
void pubOdometry(const Estimator &estimator, const std_msgs::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        // ofstream foutC(file_name, ios::app);
        ...
        ofstream foutC_tum(VINS_RESULT_PATH, ios::app);
        foutC_tum.setf(ios::fixed, ios::floatfield);
        foutC_tum.precision(9);
        foutC_tum << header.stamp.toSec() << " ";
        foutC_tum.precision(5);
        foutC_tum << estimator.Ps[WINDOW_SIZE].x() << " "
              << estimator.Ps[WINDOW_SIZE].y() << " "
              << estimator.Ps[WINDOW_SIZE].z() << " "
              << tmp_Q.x() << " "
              << tmp_Q.y() << " "
